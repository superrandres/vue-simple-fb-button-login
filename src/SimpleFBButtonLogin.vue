<template>
  <span>
    <slot :login="login">
      <button @click="login" class="fb connect">{{ textButton }}</button>
    </slot>
  </span>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    handleResponseLoginFB (response) {
      this.getLoginStatusFB(response)
      if (response.status === 'connected') {
        window.FB.api(
          '/me',
          'GET',
          {
            fields: this.fieldsGet
          },
  ...
  created () {
    ...
    this.oAuthFB = {
      ...
      redirect_uri: this.redirectURI ? this.redirectURI : path,
